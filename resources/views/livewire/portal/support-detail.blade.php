<div class="space-y-8">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div class="flex items-center gap-5">
            <!-- Back Button -->
            <a href="{{ route('portal.support') }}" class="w-12 h-12 bg-white border border-[#E2E8F0] rounded-2xl flex items-center justify-center text-[#64748B] hover:text-[#1E7CCF] transition-all shadow-sm group" wire:navigate>
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="px-4 py-2 bg-white border border-[#E2E8F0] rounded-xl flex items-center gap-2 shadow-sm">
                <span class="text-[10px] font-black text-[#94A3B8] uppercase tracking-widest">{{ __('Priority') }}:</span>
                <span class="text-xs font-black text-[#0F172A] uppercase">{{ $ticket->priority->label() }}</span>
            </div>
            <!-- Status Badge (Dynamic Color) -->
            <div class="px-4 py-2 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-sm {{ $ticket->status->badgeClasses() }}">
                {{ $ticket->status->label() }}
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div class="bg-white p-5 rounded-xl border border-[#E2E8F0] shadow-sm">
            <p class="text-[10px] font-black text-[#94A3B8] uppercase tracking-widest mb-1">{{ __('Last Activity') }}</p>
        ...
        @foreach($messages as $msg)
            <div class="flex {{ $msg->is_admin ? 'justify-start' : 'justify-end' }}">
                <div class="max-w-[85%] md:max-w-[70%] space-y-2">
                    ...
                    <!-- Message Bubble -->
                    <div class="relative p-6 rounded-3xl {{ $msg->is_admin ? 'bg-[#F1F5F9] text-[#334155] rounded-tl-none' : 'bg-[#E9F2FB] text-[#0F172A] border border-[#1E7CCF]/10 rounded-tr-none' }}">
                        <p class="text-sm md:text-base font-medium leading-relaxed">{{ $msg->content }}</p>

                        @if($msg->attachment)
                            <div class="mt-4 pt-4 border-t {{ $msg->is_admin ? 'border-gray-200' : 'border-blue-200' }}">
                                <a href="{{ Storage::url($msg->attachment) }}" target="_blank" class="inline-flex items-center gap-2 px-4 py-2 bg-white rounded-xl border border-black/5 text-xs font-bold text-[#1E7CCF] hover:shadow-md transition-all">
                    ...
                    <!-- Message Meta -->
                    <div class="flex items-center gap-2 px-2 {{ $msg->is_admin ? 'justify-start' : 'justify-end' }}">
                        @if($msg->is_admin)
                            <span class="w-5 h-5 bg-[#1E7CCF] text-white rounded-full flex items-center justify-center">
                                <i data-lucide="shield-check" class="w-3 h-3"></i>
                            </span>
                        @endif
                        <span class="text-[10px] font-black uppercase tracking-widest text-[#94A3B8]">
                            {{ $msg->is_admin ? __('Support Team') : __('You') }} • {{ $msg->created_at->format('H:i A') }}
                        </span>
                    </div>
                </div>
            </div>
    ...
    <div class="bg-white rounded-xl border-2 border-[#E9F2FB] p-2 shadow-xl shadow-blue-900/5">
        <form wire:submit.prevent="sendMessage" class="relative">
            <textarea wire:model="message" rows="4" class="w-full border-none focus:ring-0 p-8 text-sm md:text-base font-medium placeholder-[#94A3B8] rounded-[2rem]" placeholder="Type your reply here..."></textarea>

            <div class="flex flex-col md:flex-row items-center justify-between p-4 bg-[#F8FAFC] rounded-[2rem] gap-4">
                <!-- File Upload -->
                <div class="relative group">
                    <input type="file" wire:model="attachment" id="file-upload" class="hidden">
                    <label for="file-upload" class="flex items-center gap-2 px-5 py-2.5 bg-white border border-[#E2E8F0] rounded-xl text-xs font-bold text-[#64748B] cursor-pointer hover:border-[#1E7CCF] hover:text-[#1E7CCF] transition-all">
                ...
                <!-- Send Button -->
                <button type="submit" wire:loading.attr="disabled" class="w-full md:w-auto bg-[#1E7CCF] hover:bg-[#1866AD] text-white px-10 py-3.5 rounded-xl font-bold text-sm shadow-lg shadow-blue-100 flex items-center justify-center gap-2 transition-all">
    ...
    <div class="p-8 text-center bg-[#F1F5F9] rounded-[2.5rem] border border-[#E2E8F0]">
        <p class="text-sm font-bold text-[#64748B] italic uppercase tracking-widest">{{ __('This ticket is closed. Please open a new one if you need further help.') }}</p>
    </div>
    @endif
</div>
