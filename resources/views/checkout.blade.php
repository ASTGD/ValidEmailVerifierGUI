    ...
    <div class="max-w-[1280px] mx-auto px-10 py-20">
        <div class="grid lg:grid-cols-3 gap-12">
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="bg-white p-8 rounded-3xl border border-[#E2E8F0] shadow-sm">
                    @if (session('status'))
                        <div class="mb-6 rounded-2xl border border-[#E2E8F0] bg-[#F8FAFC] px-5 py-3 text-sm font-semibold text-[#334155]">
                            {{ session('status') }}
                        </div>
                    @endif
                    ...
                    @guest
                        ...
                        <p class="text-[#64748B] mb-8">{{ __('Please login or create an account to continue checkout.') }}</p>
                        <div class="flex gap-4">
                            <a href="{{ route('checkout.login', $intent) }}" class="flex-1 bg-[#1E7CCF] text-white text-center py-4 rounded-xl font-bold">{{ __('Login to Continue') }}</a>
                            <a href="{{ route('checkout.register', $intent) }}" class="flex-1 border-2 border-[#E2E8F0] text-center py-4 rounded-xl font-bold">{{ __('Create Account') }}</a>
                        </div>
                    @endguest
                    ...
                    @auth
                        ...
                        <form method="POST" action="{{ route('checkout.pay', $intent) }}">
                            @csrf
                            <button type="submit" class="w-full mt-10 bg-[#1E7CCF] text-white py-5 rounded-2xl font-bold text-xl shadow-xl shadow-blue-200">
                                {{ __('Pay Securely Now') }}
                            </button>
                        </form>
                        ...
                        @if ($canFakePay)
                            <form method="POST" action="{{ route('checkout.fake-pay', $intent) }}" class="mt-4">
                                @csrf
                                <button type="submit" class="w-full border-2 border-[#E2E8F0] py-4 rounded-2xl font-bold text-[#334155] hover:bg-[#F8FAFC]">
                                    {{ __('Fake Pay (Dev)') }}
                                </button>
                            </form>
