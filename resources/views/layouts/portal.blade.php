...
<body class="antialiased bg-[#F8FAFC] text-[#0F172A]" x-data="{ mobileNavOpen: false, sidebarOpen: true }">
    <div class="flex min-h-screen">
        ...
        <div class="flex-1 flex flex-col min-w-0 transition-[margin] duration-200" :class="sidebarOpen ? 'md:ml-72' : 'md:ml-0'">
            <header class="h-16 md:h-20 bg-white border-b border-[#E2E8F0] sticky top-0 z-30">
                <div class="h-full px-4 md:px-8 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <button type="button" class="md:hidden inline-flex items-center justify-center rounded-md border border-[#E2E8F0] bg-white p-2 text-[#64748B] hover:bg-[#F8FAFC]" @click="mobileNavOpen = true">
                            <span class="sr-only">{{ __('Open navigation') }}</span>
                            <i data-lucide="menu" class="w-5 h-5"></i>
                        </button>
                        <button type="button" class="hidden md:inline-flex items-center justify-center rounded-md border border-[#E2E8F0] bg-white p-2 text-[#64748B] hover:bg-[#F8FAFC]" @click="sidebarOpen = !sidebarOpen">
                        ...
                        <span class="text-sm font-black uppercase tracking-widest text-[#64748B]">{{ __('Customer Portal') }}</span>
                    </div>
                        ...
                        <div class="relative" x-data="{ open: false }">
                            <button @click="open = !open" @click.away="open = false" class="flex items-center gap-3 focus:outline-none">
                                <div class="text-right hidden sm:block">
                                    <p class="text-sm font-bold text-[#0F172A]">{{ auth()->user()->name ?: auth()->user()->email }}</p>
                                    <p class="text-[10px] font-bold text-[#94A3B8] uppercase tracking-tighter">{{ __('Verified Member') }}</p>
                                </div>
                                <div class="w-10 h-10 bg-[#E9F2FB] rounded-xl flex items-center justify-center text-[#1E7CCF] font-bold border border-[#1E7CCF]/10">
                                    {{ substr(auth()->user()->name ?: auth()->user()->email, 0, 1) }}
                                </div>
                                <i data-lucide="chevron-down" class="w-4 h-4 text-[#64748B]" :class="open ? 'rotate-180' : ''"></i>
                            </button>

                            <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute right-0 mt-3 w-48 bg-white rounded-2xl shadow-xl border border-[#E2E8F0] py-2 z-50" style="display: none;">
                                <a href="{{ route('profile') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-[#334155] hover:bg-[#F8FAFC] hover:text-[#1E7CCF] transition-colors" wire:navigate>
                                    <i data-lucide="user" class="w-4 h-4"></i> {{ __('My Profile') }}
                                </a>
                                <a href="{{ route('portal.settings') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-[#334155] hover:bg-[#F8FAFC] hover:text-[#1E7CCF] transition-colors" wire:navigate>
                                    <i data-lucide="settings" class="w-4 h-4"></i> {{ __('Settings') }}
                                </a>
                                <div class="border-t border-[#F1F5F9] my-1"></div>
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <button type="submit" class="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-red-500 hover:bg-red-50 transition-colors">
    ...
    <div x-show="mobileNavOpen" x-cloak class="fixed inset-0 z-40 bg-black/40 md:hidden" @click="mobileNavOpen = false"></div>
    <div
    ...
    >
        <div class="flex items-center justify-between border-b border-[#E2E8F0] px-4 py-4">
            ...
            <button type="button" class="rounded-md p-2 text-[#64748B] hover:bg-[#F8FAFC]" @click="mobileNavOpen = false">
                <span class="sr-only">{{ __('Close navigation') }}</span>
                <i data-lucide="x" class="w-5 h-5"></i>
            </button>
        </div>
        <nav class="space-y-1 px-4 py-4">
            <a href="{{ route('portal.dashboard') }}" class="flex items-center gap-2 rounded-md px-3 py-2 text-sm font-medium {{ request()->routeIs('portal.dashboard') ? 'bg-[#E9F2FB] text-[#1E7CCF]' : 'text-[#334155] hover:bg-[#F8FAFC]' }}" wire:navigate>
                {{ __('Dashboard') }}
            </a>
            <a href="{{ route('portal.upload') }}" class="flex items-center gap-2 rounded-md px-3 py-2 text-sm font-medium {{ request()->routeIs('portal.upload') ? 'bg-[#E9F2FB] text-[#1E7CCF]' : 'text-[#334155] hover:bg-[#F8FAFC]' }}" wire:navigate>
                {{ __('Verify List') }}
            </a>
            ...
            <a href="{{ route('portal.settings') }}" class="flex items-center gap-2 rounded-md px-3 py-2 text-sm font-medium {{ request()->routeIs('portal.settings') ? 'bg-[#E9F2FB] text-[#1E7CCF]' : 'text-[#334155] hover:bg-[#F8FAFC]' }}" wire:navigate>
                {{ __('Settings') }}
            </a>
            <a href="{{ route('portal.support') }}" class="flex items-center gap-2 rounded-md px-3 py-2 text-sm font-medium {{ request()->routeIs('portal.support') ? 'bg-[#E9F2FB] text-[#1E7CCF]' : 'text-[#334155] hover:bg-[#F8FAFC]' }}" wire:navigate>
                {{ __('Support') }}
            </a>
        </nav>
        <div class="border-t border-[#E2E8F0] px-4 py-4">
            ...
            <form method="POST" action="{{ route('logout') }}" class="mt-3">
                @csrf
                <button type="submit" class="w-full rounded-md bg-[#1E7CCF] px-3 py-2 text-sm font-semibold text-white hover:bg-[#1866AD]">
                    {{ __('Log Out') }}
                </button>
            </form>
        </div>
