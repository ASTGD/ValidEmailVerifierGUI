...
1. Ensure named tunnels exist: `app-dev`, `go-dev`
2. Start both:
   - `cloudflared tunnel run --url http://localhost:8082 app-dev`
   - `cloudflared tunnel run --url http://localhost:9091 go-dev`
3. Public URLs:
   - Laravel app: `https://app-dev.astgd.com`
   - Go control plane: `https://go-dev.astgd.com`
...
- `./scripts/restart-go-dashboard.sh`
- `./scripts/run-cloudflare-tunnels.sh`
- `./scripts/run-go-worker.sh` (manual/local only)
- `./scripts/run-ngrok.sh` (legacy/optional)
...
- Pane 5: `./scripts/run-cloudflare-tunnels.sh`
...
- Does not start local `engine-worker-go` (real VPS workers are primary).
- `run-queue.sh` starts Horizon (segmented queue workers).

## Restart only Go dashboard
